      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
        {[
          ...
          <div key={i} className={`glass-card p-8 flex flex-col justify-between shadow-premium border-l-4 border-${stat.color}`}>
            <p className="text-[12px] font-black text-on-surface-variant uppercase tracking-[0.2em] mb-4">{stat.label}</p>
            <div className="flex items-end justify-between">
              <h3 className={`font-headline text-3xl font-black text-${stat.color}`}>{stat.val}</h3>
              <div className="flex flex-col items-end">
                {stat.change && (
                  <span className={`${stat.status === 'success' ? 'text-secondary' : 'text-error'} text-[12px] font-black flex items-center gap-1`}>
                    <span className="material-symbols-outlined text-base">{stat.icon}</span>{stat.change}
                  </span>
                )}
                {stat.sub && <span className="text-on-surface-variant text-[11px] font-black uppercase tracking-widest">{stat.sub}</span>}
              </div>
            </div>
          </div>
        ))}
      ...
      <div className="glass-card shadow-premium overflow-hidden border-white/5">
        <div className="p-8 border-b border-outline-variant/10 flex flex-col sm:flex-row items-center justify-between gap-6">
          <div className="flex items-center gap-2">
            <button className="btn-secondary !py-2 !px-4 !text-[12px] uppercase tracking-widest">
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-surface-container-highest/10 border-b border-outline-variant/10">
                ...
                <th className="px-8 py-5 text-[12px] font-black uppercase tracking-[0.2em] text-on-surface-variant">Product Logic</th>
                <th className="px-8 py-5 text-[12px] font-black uppercase tracking-[0.2em] text-on-surface-variant">Lifecycle</th>
                <th className="px-8 py-5 text-[12px] font-black uppercase tracking-[0.2em] text-on-surface-variant">Fit Dynamics</th>
                <th className="px-8 py-5 text-[12px] font-black uppercase tracking-[0.2em] text-on-surface-variant text-center">Score Delta</th>
                <th className="px-8 py-5 text-[12px] font-black uppercase tracking-[0.2em] text-on-surface-variant text-right">Inventory</th>
              ...
              {products.map((p) => (
                <tr key={p.id} className={`group hover:bg-surface-container-highest/10 transition-all ${p.status === 'Low Stock' ? 'bg-error/5 hover:bg-error/10' : ''}`}>
                  ...
                  <td className="px-8 py-6">
                    <div className="flex items-center gap-5">
                      ...
                      <div className="space-y-1">
                        <p className="font-bold text-[15px] text-on-surface leading-tight underline decoration-primary/10 group-hover:decoration-primary/40 underline-offset-4">{p.name}</p>
                        <p className="text-[11px] font-black text-on-surface-variant uppercase tracking-widest">SKU: {p.sku}</p>
                      </div>
                    </div>
                  </td>
                  <td className="px-8 py-6">
                    <span className={`px-2.5 py-1 rounded text-[11px] font-black uppercase tracking-[0.15em] border ${
                  ...
                  <td className="px-8 py-6 text-center">
                    <span className={`text-[13px] font-black uppercase tracking-widest ${parseFloat(p.returnRate) > 10 ? 'text-error' : 'text-secondary'}`}>{p.returnRate}</span>
                  </td>
                  <td className="px-8 py-6 text-right">
                    <p className="text-base font-black text-on-surface">{p.stock}</p>
                    <p className="text-[10px] font-black text-on-surface-variant uppercase tracking-widest mt-1">units</p>
                  </td>
                  <td className="px-8 py-6 text-right">
                    ...
                    <div className="flex justify-end mt-1.5">
                      <div className="flex items-center gap-1 px-2 py-0.5 rounded bg-tertiary/10 border border-tertiary/10">
                        ...
                        <span className="text-[11px] font-black text-tertiary">{p.rating}</span>
                      </div>
                    </div>
                  </td>
                </tr>
        ...
        <div className="p-8 border-t border-outline-variant/10 flex flex-col sm:flex-row items-center justify-between gap-6 transition-colors">
          <p className="text-[12px] font-black text-on-surface-variant uppercase tracking-[0.15em]">Showing <span className="text-on-surface">1-4</span> of <span className="text-on-surface">254</span> identifiers</p>
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-1.5">
              ...
              <button className="w-10 h-10 flex items-center justify-center rounded-lg hover:bg-surface-container-highest/20 font-black text-[12px] transition-all border border-transparent hover:border-outline-variant/10">2</button>
              <button className="w-10 h-10 flex items-center justify-center rounded-lg hover:bg-surface-container-highest/20 font-black text-[12px] transition-all border border-transparent hover:border-outline-variant/10">3</button>
            </div>
